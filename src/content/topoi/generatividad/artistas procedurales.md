...
process art
warp-weft
expression-composition pattern

## code design patterns
flat functionals namespace (all logic lives in a top level immutable vars)
**const-based**
	immediate procedural (every const is both definition and executable statement)
aesthetics functional minimalism (no OOP, no side effects)
	functional programming (λ-calculus → JS closures
point-free style (no arguments unless necessary);
generative art “tweetcode” or code-golf minimalism (compact, declarative)
no globals 
functional purity →  each step is declarative data flow; 
