...
const pullFromLocalPath = (source, targetDir) => {
  ...
  if (!fs.existsSync(localPath)) {
    throw new Error(`Source "${source.id}" localPath does not exist: ${localPath}`);
  }

  if (!fs.existsSync(targetDir)) {
    run('git', ['clone', '--depth', '1', localPath, targetDir]);
  } else {
...
const main = () => {
  ...
  for (const source of sources) {
    ...
    if (source.localPath) {
      pullFromLocalPath(source, targetDir);
    } else {
      pullFromRepo(source, targetDir, token);
    }

    const contentRoot = source.contentRoot || 'content';
    const contentPath = path.join(targetDir, contentRoot);
    if (!fs.existsSync(contentPath)) {
      throw new Error(
        `Source "${source.id}" is missing content root "${contentRoot}" at ${contentPath}`,
      );
    }
