...
async function ragDecision(state, choices){
  ...
  try{
    if(RAG.llm==="ollama"){
      ...
      const res=await fetch("https://localhost:1234/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json"},
...
/* ---------------- CONFIG ---------------- */
const OLLAMA_URL = "https://localhost:11434";
