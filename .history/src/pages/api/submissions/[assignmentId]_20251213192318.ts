...
// DELETE submission - params.assignmentId is actually the submission ID here
export const DELETE: APIRoute = async ({ params, locals }) => {
  ...
  // In this context, assignmentId is actually the submission ID
  const submissionId = params.assignmentId;
  if (!submissionId) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Error al borrar la respuesta" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
