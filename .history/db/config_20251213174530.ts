...
const Enrollment = defineTable({
  columns: {
    ...
    enrolledAt: column.date({ default: NOW }),
  },
  indexes: {
    userCourse: { on: ['userId', 'courseId'], unique: true }
  }
});
...
const Assignment = defineTable({
  columns: {
    ...
    slug: column.text(), // El 'slug' de tu archivo .mdx (multiple eval blocks can share same slug)
